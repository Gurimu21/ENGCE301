export async function searchMovie(search_data) {
    try {
        ...
        const response = await fetch('http://localhost:3001/api/movie/search?title=' + search_data);
...
export async function deleteMovie(delete_data) {
    try {
        ...
        const response = await fetch('http://localhost:3001/api/movie/delete?title=' + delete_data, {method:'DELETE'});
        return await response.json();
    } catch (error) {
        return [];
    }
